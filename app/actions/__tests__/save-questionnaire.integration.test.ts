...
describe("Questionnaire Integration Tests", () => {
  ...
  const createTestFormState = (overrides?: Partial<GlobalFormState>): GlobalFormState => {
    return {
      currentStep: 5,
      ...
      propertyIssues: {
        ...
        positiveAspects: ["Logement récemment rénové", "Quartier très bien desservi"],
        additionalComments:
  ...
  describe("saveQuestionnaireResponse", () => {
    ...
    it("should handle high rent overpayment case", async () => {
      ...
      if (result.submissionId) {
        ...
        expect(Number(submission?.actual_rent) - Number(submission?.max_rent)).toBe(565);
      }
    });
    ...
    it("should store complex property issues correctly", async () => {
      ...
      if (result.submissionId) {
        ...
        const { data: submission } = await supabaseAdmin
          .from("questionnaire_responses")
          .select("health_issues, major_defects, positive_aspects, additional_comments")
          .eq("id", result.submissionId)
          .single();
    ...
    it("should handle special characters in text fields", async () => {
      ...
      if (result.submissionId) {
        ...
        expect(submission?.additional_comments).toContain("caractères spéciaux");
      }
    });
  });
